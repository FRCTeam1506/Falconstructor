...
waypoint_cnt = int(input("Number of Waypoints in trajectory: "))

interior = []
interior_waypoint_cnt = waypoint_cnt - 2
...
for i in range(interior_waypoint_cnt):
    if(len(data) % 2 == 0):
        mid = data[ (i + 1) * ((len(data) // (waypoint_cnt - 1)) + 2) ]
    else:
        mid = data[ (i + 1) * ((len(data) // (waypoint_cnt - 1)) + 1) ]
    interior.append(mid)
